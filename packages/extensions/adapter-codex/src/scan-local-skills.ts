...
/**
 * Scan `<targetDir>/.agents/skills/` for user-managed skills and return
 * one entry per directory containing a `SKILL.md`. Codex discovers skills by
 * walking `.agents/skills` from the working directory up to the repository
 * root, so these are always active regardless of AIR's involvement — the TUI
 * surfaces them as read-only entries. Missing or unreadable directories yield
 * an empty list — this is a best-effort informational scan, not validation.
 */
