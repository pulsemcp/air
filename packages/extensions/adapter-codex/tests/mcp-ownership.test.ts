...
describe("CodexAdapter MCP server ownership (#174)", () => {
  ...
  describe.each([1, 2])("a version %i manifest (written before the fix)", (version) => {
    ...
    it("prints a server's translation warnings once, not again while checking ownership", async () => {
      const artifacts = catalog();
      artifacts.mcp["@local/notion"] = {
        type: "streamable-http",
        url: "https://mcp.notion.example/mcp",
      ...
      await select(artifacts, ["notion"]);
      expect(manifestServers()).toEqual(["notion"]);
      const translationWarnings = warn.mock.calls.filter((args) =>
        String(args[0]).includes("X-Api-Key")
      );
      expect(translationWarnings).toHaveLength(1);
    });
  });
