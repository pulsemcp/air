...
function createTempDir(): string {
  const dir = join(
    tmpdir(),
    `air-codex-scan-skills-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  mkdirSync(dir, { recursive: true });
  return dir;
}

describe("scanLocalSkills (codex)", () => {
  ...
  it("returns empty list when .agents/skills does not exist", () => {
  ...
  it("returns empty list when the target directory does not exist", () => {
    expect(
      scanLocalSkills(join(tmpdir(), `air-codex-nonexistent-${Date.now()}`))
    ).toEqual([]);
  });
  ...
  it("skips dotfiles in .agents/skills/", () => {
    const dir = createTempDir();
    cleanup.push(dir);
    const skillsDir = join(dir, ".agents", "skills");
