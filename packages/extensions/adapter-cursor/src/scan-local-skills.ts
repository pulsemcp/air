...
/**
 * Scan `<targetDir>/.cursor/skills/` for user-managed skills and return
 * one entry per directory containing a `SKILL.md`. Cursor discovers skills by
 * scanning `.cursor/skills/` (and `.agents/skills/`) from the working
 * directory, so these are always active regardless of AIR's involvement — the
 * TUI surfaces them as read-only entries. Missing or unreadable directories
 * yield an empty list — this is a best-effort informational scan, not
 * validation.
 */
