...
function createTempDir(): string {
  const dir = join(
    tmpdir(),
    `air-cursor-scan-skills-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  mkdirSync(dir, { recursive: true });
  return dir;
}

describe("scanLocalSkills (cursor)", () => {
  ...
  it("returns empty list when .cursor/skills does not exist", () => {
  ...
  it("returns empty list when the target directory does not exist", () => {
    expect(
      scanLocalSkills(join(tmpdir(), `air-cursor-nonexistent-${Date.now()}`))
    ).toEqual([]);
  });
  ...
  it("skips dotfiles in .cursor/skills/", () => {
    const dir = createTempDir();
    cleanup.push(dir);
    const skillsDir = join(dir, ".cursor", "skills");
