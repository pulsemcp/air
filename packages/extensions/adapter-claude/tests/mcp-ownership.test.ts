...
beforeEach(() => {
  base = mkdtempSync(join(tmpdir(), "air-claude-mcp-ownership-"));
...
function writeConfig(config: Record<string, unknown>): void {
  write(join(target, CONFIG_FILE), JSON.stringify(config, null, 2) + "\n");
}

function servers(): Record<string, Record<string, unknown>> {
  return (readConfig()?.mcpServers ?? {}) as Record<string, Record<string, unknown>>;
}

function setServers(next: Record<string, unknown>): void {
  writeConfig({ ...(readConfig() ?? {}), mcpServers: next });
}
...
describe("ClaudeAdapter MCP server ownership (#174)", () => {
  ...
  describe.each([1, 2])("a version %i manifest (written before the fix)", (version) => {
    ...
    // Claude only: the secret transforms rewrite `.mcp.json` in place.
    it("keeps owning a key whose ${VAR} placeholders a secrets transform has since resolved", async () => {
      const artifacts = catalog();
      ...
      await select(artifacts, ["slack", "notion"]);
      editServer("slack", { env: { SLACK_TOKEN: "xoxb-resolved" } });
      editServer("notion", { headers: { Authorization: "Bearer resolved" } });
      rewriteAsLegacy(version, ["slack", "notion"]);
      const warn = silenceWarnings();

      await select(artifacts, []);
      expect(servers().slack).toBeUndefined();
      expect(servers().notion).toBeUndefined();
      expect(warn).not.toHaveBeenCalled();
    });

    it("gives up a key whose text around a placeholder changed", async () => {
      const artifacts = catalog();
      artifacts.mcp["@local/notion"] = {
        type: "streamable-http",
        url: "https://mcp.notion.example/mcp",
      ...
      editServer("notion", { headers: { Authorization: "Token mine" } });
