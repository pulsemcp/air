...
- **`air start`'s interactive selector now opens on what is installed in the current directory, not on the root's defaults.** Before this, on a second `air start claude` the TUI ignored the previous run: a non-default skill the user had picked (and AIR had written to `.claude/skills/`) was not preselected, a default the user had left out was preselected again, and the skills AIR had written showed up a second time as locked 🔒 "local" rows. Pressing Enter on that screen reinstalled the defaults and removed the earlier picks, because the adapters already reconcile each run against AIR's per-directory manifest. Now:
  - The new SDK helper `getInstalledArtifacts({ target, adapter, artifacts, prefer? })` reads that manifest and returns the skills, MCP servers, and hooks AIR installed there, as qualified catalog IDs. It also returns every plugin whose declared skills, MCP servers, and hooks are all installed. It returns `null` when the directory has no manifest, or when a different adapter wrote the manifest. `air start` uses the result for the TUI's preselection and falls back to root defaults when it is `null` (the first run in a directory). Pressing Enter without changing anything now leaves the directory as it is.
  - `startSession()` no longer reports skills that AIR installed as `localArtifacts`, so they stay toggleable instead of appearing as locked local skills. Skills that really are checked into the repo still appear as locked rows, as before.
  - Only the TUI's preselection changes. Non-interactive runs (`--skip-confirmation`, artifact flags, no TTY) still use root defaults, and what a run removes from disk is still decided by the adapters' existing manifest reconciliation, which this change does not touch. Addresses the first ask of [#122](https://github.com/pulsemcp/air/issues/122).
