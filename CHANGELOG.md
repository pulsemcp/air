...
### Added
- **`air start`'s interactive selector marks the root's defaults with ★, separately from what is selected.** Since the selector began opening on what is installed rather than on the root's defaults (below), a user who deselected a default, or whose directory drifted from the root, had no way to see which items the root recommends. Now every row in the root's defaults shows a ★ after its ID, on all four tabs (skills, MCP servers, hooks, plugins). A `★ root default` legend line above the list, in the style of the 🔒 local-skills line, explains it. Defaults are the active root's, merged with its subagent roots' unless `--no-subagent-merge` is passed, the same set a first run preselects. The marker is display-only: selection, preselection, what `getSelectedIds` hands to `prepareSession`, and non-interactive runs (`--skip-confirmation`, artifact flags, no TTY) are unchanged. The selector now also reserves terminal rows for these legend lines, including the existing 🔒 line, so showing both no longer pushes the key legend off the bottom of a full screen. Addresses the third ask of [#122](https://github.com/pulsemcp/air/issues/122).
- **Test coverage and docs for removing a deselected item.** Deselecting a skill, MCP server, hook, or plugin that AIR installed, then pressing Enter, removes it from the directory. The adapters' existing manifest reconciliation already did this; no deletion logic changed. A new round-trip test in `packages/cli/tests/tui-installed-state.test.ts` pins it through the real SDK and Claude adapter. It also checks that 🔒 local skills and other user-authored items stay: pre-existing skill and hook directories named like the deselected catalog items, a user `.mcp.json` key, and a user settings hook. `running-sessions.md` and `managing-skills.md` now say so. They also documented one exception that predated this change, a user-added `.mcp.json` key replaced by a catalog MCP server with the same name and removed when that server was deselected; that is now fixed ([#174](https://github.com/pulsemcp/air/issues/174), under Fixed). Addresses the second ask of [#122](https://github.com/pulsemcp/air/issues/122).

### Changed
- **`air update` and `air upgrade` are now one command, and no version is ever bumped without consent.** The two used to do disjoint halves of the same job — `air update` refreshed cached provider data but never touched the CLI, `air upgrade` ran `npm install -g` but never refreshed a cache — and the boundary was invisible until a user hit it, leaving them in a silent partial state. `air update` is now the canonical name and does the whole job: it refreshes provider caches, then checks the CLI **and** every extension declared in `air.json` for newer published versions. Resolves [#132](https://github.com/pulsemcp/air/issues/132).
  - **The cache refresh always runs**, exactly as before — it changes no versions, so there is nothing to consent to. **The version check never installs on its own initiative.** When a bump is available it prints a table of what would change and asks; declining installs nothing.
  - **Non-TTY defaults to no upgrade.** When stdin or stdout is not a terminal — CI, a pipeline, a scripted wrapper — the upgrade is skipped and the reason printed, rather than an unattended `npm install -g` running. This holds by construction: the SDK's new `runUpdate()` installs only when given `assumeYes` or a `confirm` callback, and the CLI supplies that callback only when `isInteractiveTTY()`. Pass `--yes` to opt a script in.
  - New flags: `-y` / `--yes` (assume confirmation), `--no-upgrade` (refresh caches only; still reports available bumps, and outranks `--yes`), and `--dry-run`. `--no-extensions`, `--no-auto-heal`, `--git-protocol` and `--config` are carried over from the two old commands, so both old invocations keep working.
  - **`air upgrade` still works.** It prints a deprecation notice naming `air update`, then does the same work with the same flags. Deprecating is not removing — existing scripts and muscle memory keep working, though a non-interactive `air upgrade` now needs `--yes` to install, which is the point of the change.
  - **A failed cache refresh no longer aborts the run.** A provider too old or too broken to load is precisely the state the version check repairs, so the failure is reported and the run carries on to the step that can fix it. `air update` exits 1 only when nothing repaired it.
  - All `npm` invocations moved out of the CLI and behind the SDK's `runUpdate()`, keeping the CLI a formatting-and-prompting layer.

### Removed
- **BREAKING: a plugin's body can no longer be declared inline on its `plugins.json` entry — it must live in the `.plugin/plugin.json` manifest the entry points at with `path`.** This form was deprecated in 0.13.0, where it resolved with a warning; the warning is now a hard error. Resolves [#157](https://github.com/pulsemcp/air/issues/157).
  - **What is rejected.** A plugin entry that carries any body field (`title`, `version`, `skills`, `mcp_servers`, `hooks`, `plugins`, `author`, `homepage`, `repository`, `license`, `logo`, `keywords`) with **no sibling `path`**. `resolveArtifacts` raises `CatalogConfigError` naming the plugin, every offending field, and where they belong; `plugins.schema.json` rejects the same shape, so `air validate` reports it (with the same message, not AJV's raw one-field-at-a-time `dependencies` complaint) before resolution ever runs.
  - **What still works, unchanged.** Inline fields layered **on top of** a `path` — the sanctioned per-field override of the manifest — were never deprecated and still resolve: the entry wins, the manifest fills the gaps. So does an entry that declares no body at all (`description` + optional `default_in_roots`); the rule is about body fields without a `path`, not about `path` being mandatory on every entry. And a plugin whose manifest is missing, unparseable, or invalid is still isolated to a warning that drops only that plugin, exactly as in 0.13.1.
  - **Why a hard error rather than warn-and-drop.** Per-source and per-plugin isolation exists for a catalog's *content* failing in transit — a half-written manifest, an unreachable provider. An inline body is an author mistake in a catalog that was explicitly listed, in the same class as a catalog URI whose scheme has no installed provider extension, which has hard-failed since 0.13.1. Degrading it to a warning would silently drop the plugin — and every skill, MCP server, and hook it bundles — from every session that composes that catalog, which is the outcome this removal exists to make visible.
  - **Migration** (mechanical, and the same one the deprecation warning has been printing since 0.13.0): for each affected plugin, move its body fields into `<plugin-dir>/.plugin/plugin.json`, and replace them on the index entry with `"path": "./<plugin-dir>"`. Keep `description`, `path`, and `default_in_roots` on the index entry — `description` because the registry needs it to list the plugin, `default_in_roots` because root membership is a catalog-layer decision rather than a property of the distributed plugin. The manifest's reference arrays stay bare shortnames; they qualify under the catalog's scope exactly as the inline ones did.
  - **Compatibility.** This is deliberately breaking for catalog authors (pre-1.0): a `plugins.json` that validates today can stop validating, and one that resolves today can stop resolving, with no deprecation period left to run. Pinned installs are unaffected until the pin moves. **It reaches consumers too, not just authors:** an inline body in a `github://` catalog you merely *compose* fails your resolve exactly as a local one does, and `exclude` cannot rescue it (exclusion runs after the catalogs are loaded). That is the intended trade — the alternative silently removes the plugin, and everything it bundles, from every session — but if upstream has not migrated, the remedies are on your side: pin that catalog to a pre-0.14 ref, vendor or fork it, or drop it from `air.json` until it migrates. `docs/plugins.md`, `docs/guides/composition-and-overrides.md`, the root `README.md`, and the `examples/` and e2e fixture catalogs now show only the manifest form.

### Fixed
- **Adapters no longer overwrite, or later remove, a user's MCP server key that shares a catalog server's name.** In the `claude`, `codex` and `cursor` adapters, selecting a catalog MCP server replaced a user-written entry under the same key in `.mcp.json` / `.codex/config.toml` / `.cursor/mcp.json` and recorded the key in the per-target manifest as AIR's, so deselecting the server later deleted the entry for good. Now `prepareSession` writes a selected server only into a free key or over a key the previous manifest owns. A key AIR didn't write stays exactly as it is and is kept out of the manifest, and AIR prints a warning that it did not write that catalog server. Skill and hook directories already worked this way. `pi` writes no MCP config and is unaffected. Resolves [#174](https://github.com/pulsemcp/air/issues/174).
  - **Manifests from earlier versions.** Manifests are now written as `version: 3` (core's new `manifestMcpServersAreAirOwned()`). Every earlier manifest, `version: 2` included, may list a user's key, so the first `air prepare` or `air start` after upgrading checks each MCP server such a manifest lists whose key is still present. It stays AIR's only if the entry is what the adapter writes for a catalog server with that name. In Claude's `.mcp.json` a `${VAR}` placeholder may hold any value, since the secrets transforms resolve placeholders there in place; Codex and Cursor configs must match exactly. Anything else is left in place, dropped from the manifest, and reported in a warning: that includes an AIR-written entry you edited, and one whose catalog server changed after AIR wrote it. Delete such an entry by hand to let AIR manage the catalog server again.
  - **`air clean`** has no catalog to check against. For a manifest older than `version: 3` it leaves the listed MCP server keys in place, keeps them in the manifest at the old version, and warns; one `air prepare` or `air start` sorts them out.
- **Adapters no longer claim a skill directory they didn't create, so leaving a catalog skill out of a run can't delete a user's own skill with the same name.** In all four adapters (`claude`, `codex`, `cursor`, `pi`), `prepareSession` skipped copying over a skill directory that already existed, but still recorded it in the per-target manifest as AIR's. The next run that left that catalog skill out then deleted the directory (`rm -rf`), uncommitted edits included. Now a directory that already exists is recorded again only if the previous manifest owned it. Hooks already had this guard. A manifest written by a different adapter is now ignored rather than acted on, since its entries name that adapter's directories; switching adapters in one directory could otherwise claim or delete same-named skills, hooks or MCP servers. Resolves [#168](https://github.com/pulsemcp/air/issues/168).
  - **Manifests from earlier versions.** A manifest written by AIR 0.13.1 or earlier may already list a user's directory. Trusting those entries would keep deleting it, and distrusting all of them would orphan every skill AIR installed. So manifests are now written as `version: 2` (core's new `manifestSkillsAreAirOwned()` tells the two apart), and each skill a `version: 1` manifest lists is checked once. It stays AIR's only if its files are byte-for-byte what AIR installs for a catalog skill with that shortname. Any other entry is left on disk, dropped from the manifest, and named in a warning. That includes an AIR copy the user edited, or one whose catalog skill has changed since: AIR can't vouch for those either, and it no longer deletes what it can't vouch for. **Upgrade note:** AIR never refreshes an installed skill, so with a `github://` catalog that has moved on, expect a warning per long-lived copy on the first run after upgrading. Each such copy is left on disk as a local skill; delete the ones you don't need.
  - **`air clean`** has no catalog to check against. For a `version: 1` manifest it leaves the listed skill directories in place, keeps them in the manifest (still `version: 1`), and warns; one `air prepare` or `air start` sorts them out. Hooks are cleaned as before; for MCP servers, see the entry above.
  - With this, the `air start` change below classifies checked-in skills exactly: they are never in the manifest, so they stay 🔒 local and are never preselected as installed.
- **`air start`'s interactive selector now opens on what is installed in the current directory, not on the root's defaults.** Before this, on a second `air start claude` the TUI ignored the previous run: a non-default skill the user had picked (and AIR had written to `.claude/skills/`) was not preselected, a default the user had left out was preselected again, and the skills AIR had written showed up a second time as locked 🔒 "local" rows. Pressing Enter on that screen reinstalled the defaults and removed the earlier picks, because the adapters already reconcile each run against AIR's per-directory manifest. Now:
  - The new SDK helper `getInstalledSelection({ target, adapter, artifacts, defaults? })` reads that manifest and returns a selection that reproduces what AIR installed there, in qualified catalog IDs. Handing it back to `prepareSession` leaves the directory unchanged. It covers every installed skill, MCP server, and hook, plus every *default* plugin whose primitives are all installed. The primitives such a plugin provides are left to the plugin unless they are defaults themselves, so deselecting the plugin still removes them. The manifest doesn't record plugins, so a non-default plugin is never inferred; its primitives are selected individually instead.
  - The helper returns `null` when the directory has no manifest, when a different adapter wrote the manifest, or when AIR home can't be resolved. It leaves a category `undefined` when an installed shortname matches catalog entries in more than one scope and the defaults don't settle which. `air start` falls back to root defaults in both cases, which covers the first run in a directory.
  - `startSession()` no longer reports skills that AIR installed as `localArtifacts`, so they stay toggleable instead of appearing as locked local skills. It also now returns `adapterName`. A checked-in skill that an earlier AIR version recorded in the manifest ([#168](https://github.com/pulsemcp/air/issues/168)) can't be told apart from an AIR copy, so it shows as a toggleable, preselected row instead of a 🔒 one until the next run checks that manifest (see the #168 entry above).
  - Only the TUI's preselection changes. Root defaults now shape only the first interactive run in a directory, even when `--root` is passed. Non-interactive runs (`--skip-confirmation`, artifact flags, no TTY) still use root defaults, and what a run removes from disk is still decided by the adapters' existing manifest reconciliation, which this change does not touch. Addresses the first ask of [#122](https://github.com/pulsemcp/air/issues/122).
...
- **`@pulsemcp/air-provider-github` — a cached clone of a *mutable* ref (`HEAD` or a branch name) is now refreshed once it is older than a 5-minute TTL, instead of being served for the lifetime of the cache directory.** Previously `ensureClone()` short-circuited on the mere existence of `<cloneDir>/.git`, and nothing on the read path ever ran `git fetch` — `resolve()` and `resolveCatalogDir()` never called `checkFreshness()`, and `checkFreshness()` only *warned*. A `HEAD` clone populated at 8 AM therefore kept serving that snapshot all day, even after new commits landed upstream; only an explicit `air update` or deleting `~/.air/cache/github/` could move it. Now, when the ref is non-SHA and the clone is past its TTL, the next read performs `git fetch --depth 1 origin` + `git reset --hard origin/HEAD` (or `origin/{ref}`'s `FETCH_HEAD` for a named branch) before serving the clone.
  ...
  **This is a behavior change**: a long-lived process can now observe newer catalog content mid-session, where before it was pinned to whatever it first cloned. That is the intent — a mutable ref that never moves is the bug — but consumers that relied on within-process immutability should pin a full commit SHA instead.
  ...
  - **The refresh is best-effort, and cheap when nothing changed.** After fetching, the clone is hard-reset **only if the remote actually moved**; on the common TTL expiry that finds no new commit, the working tree is never touched. That matters beyond speed: `git reset --hard` rewrites the *published* clone in place, and a reader inside its own TTL walks that directory without taking the lock, so not touching it unless there is genuinely new content to serve shrinks the exposure from "every TTL expiry" to "only when the refresh has something to deliver". The initial clone keeps its full temp-dir-then-rename atomicity; a refresh does not, and the provider's `AGENTS.md` now says so explicitly rather than claiming an invariant that no longer holds.
  - **Failure is never fatal.** Offline, expired auth, lock contention, or a cache directory removed underfoot all log a warning and serve the cached bytes rather than failing the caller's `resolve()`. For the same reason the read-path refresh makes a single attempt instead of going through `withGitRetry` (whose ~35s of backoff would be spent on a read whose fallback is already correct), and bounds its fetch at 15s (`REFRESH_FETCH_TIMEOUT_MS`) rather than the 60s `CLONE_TIMEOUT_MS` that bounds a clone the caller cannot proceed without — so a captive portal costs a resolve seconds, not a minute per catalog.
  - **Only a 40-hex SHA counts as immutable.** A tag is deliberately treated as mutable, because git tags can be force-moved and a moved tag served forever is the exact bug being fixed; the cost is one cheap `fetch` per TTL window with no reset when the tag has not moved.
  ...
  - **Tunable via `AIR_GIT_CACHE_TTL_MS`** (default `300000`, documented in `docs/cli.md`); `0` re-checks on every resolve, a large value effectively pins the cache to the old behavior. `checkFreshness()` deliberately stays read-only — it is the SDK's warning report, not a mutation path.
  - **Credential redaction was widened** to strip URL userinfo (`https://<token>@github.com/...`) in addition to the token this process holds. An HTTPS clone persists that URL in `.git/config`, so git echoes it — possibly embedding a since-rotated token — in `unable to access` errors, and this change puts that error text on the read path where it can surface from any `resolve()`.
...
### Fixed
- **A single catalog's malformed content no longer aborts the entire `air prepare` / `resolveArtifacts`.** Resolution now isolates per-source failures so that one broken catalog — or one broken plugin within a catalog — degrades to a warning and is dropped, while every other catalog and plugin resolves normally. This matters in multi-catalog setups (e.g. Agent Orchestrator, which layers several `github://` catalogs): previously a mid-migration plugin whose externalized `.plugin/plugin.json` manifest was missing, unparseable, or schema-invalid raised a fatal `Error` that wedged setup for *every* session — including sessions that don't use the offending catalog or plugin at all. Two layers changed:
  - **Per-plugin manifest hydration is isolated.** When a plugin declares a `path`, a missing/unparseable/invalid `<path>/.plugin/plugin.json` manifest now pushes a warning naming the plugin and drops only that plugin, instead of throwing and aborting resolution of every sibling plugin and catalog.
  - **Per-source catalog loading is isolated.** When an artifact index file cannot be loaded or parsed (unreachable provider, malformed JSON, a bad `path` inside the index), AIR logs a warning naming the index, skips that one source, and continues loading the rest. The one deliberate exception is a structural `air.json` mistake — a catalog URI whose scheme has no installed provider extension — which is raised as a new `CatalogConfigError` and still hard-fails, because silently dropping a whole catalog the author explicitly listed would hide a configuration error rather than tolerate a content problem.

  The inline-plugin-body deprecation introduced in 0.13.0 remains a non-fatal warning and is unchanged — the inline form still resolves during the deprecation window. `CatalogConfigError` is exported from `@pulsemcp/air-core` for consumers that want to distinguish author-misconfiguration from content failures.

## [0.13.0] - 2026-06-12

### Deprecated
- **Declaring a plugin's body inline on the `plugins.json` entry (no `path`) is now deprecated in favor of the `.plugin/plugin.json` manifest.** When a plugin entry has no `path` but carries any body field (`title`, `version`, `skills`, `mcp_servers`, `hooks`, `plugins`, `author`, `homepage`, `repository`, `license`, `logo`, `keywords`), `resolveArtifacts` emits a deprecation warning that names the plugin, lists the offending inline fields, and links the tracking issue. The fix is mechanical: move the body into `<plugin-dir>/.plugin/plugin.json`, point the entry at it with `path`, and keep `description`, `path`, and `default_in_roots` on the index record. Inline fields layered **on top of** a `path` (overriding individual manifest fields) are the sanctioned override path and are **not** deprecated — they emit no warning. The inline-only form still resolves during the deprecation window; it is slated for removal in a future release, tracked in [pulsemcp/air#157](https://github.com/pulsemcp/air/issues/157). The plugins index schema marks the affected fields `deprecated`, and the bundled `examples/plugins/` now externalize every plugin body so the examples model the recommended form.

### Added
- **A plugin entry can now externalize its body into a `.plugin/plugin.json` manifest referenced by `path`, keeping `plugins.json` a lightweight registry of `name` + `description` + `path` (+ `default_in_roots`).** Previously a plugin was the only AIR artifact with no body — every field (artifact references *and* distribution metadata: `author`, `version`, `homepage`, `repository`, `license`, `logo`, `keywords`, `title`) had to live inline on the `plugins.json` entry, so a catalog with many plugins (or a plugin that joins dozens of roots via `default_in_roots`) bloated the index. A plugin entry may now set `path` (resolved exactly like the `path` field on skills, references, and hooks — a relative path against the index file's directory, or a provider URI such as `github://owner/repo[@ref]/dir`); `resolveArtifacts` reads the body at `<path>/.plugin/plugin.json` and merges its fields into the entry before scope-qualification, so the manifest's `skills` / `mcp_servers` / `hooks` / `plugins` reference arrays canonicalize under the catalog's scope and expand identically to inline declarations. This is AIR's vendor-neutral analog of the [Open Plugins](https://open-plugins.com/plugin-builders/specification) `.plugin/plugin.json` manifest — same file location, but the component fields hold AIR artifact **IDs** rather than paths to bundled component directories, preserving cross-plugin DRY and the CLI's ability to reason about overlap. Merge precedence is **inline-wins**: any field declared on the `plugins.json` entry overrides the manifest (the manifest only fills gaps); this composes a single plugin's split definition and is *not* cross-catalog later-wins. `description`, `path`, and `default_in_roots` always live on the index entry — `description` because the registry needs it to list the plugin, and `default_in_roots` because root membership is a catalog-layer decision, not a property of the distributed plugin. The manifest's optional `name` is accepted for Open Plugins compatibility but ignored for identity (the authoritative ID remains the `plugins.json` entry key). The feature is backward-compatible and opt-in: a plugin may declare everything inline (no `path` — now **deprecated**, see below), externalize everything (thin index + manifest — recommended), or set `path` and override individual fields inline. A missing `<path>/.plugin/plugin.json`, unparseable JSON, or a reference field that is not an array of strings fails resolution with a diagnostic that names the plugin. Adds a new `schemas/plugin-manifest.schema.json` (the AIR Plugin Manifest schema), a `path` field on the plugins index schema, and a `path` property on the core `PluginEntry` type.
