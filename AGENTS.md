...
- **Adapter extensions** translate AIR artifacts into agent-specific formats. The Claude adapter writes `.mcp.json` and injects skills via `prepareSession()`; the Codex adapter writes `.codex/config.toml` and injects skills into `.agents/skills/`; the Pi adapter is skills-only and injects skills into `.pi/skills/` with no config file.
