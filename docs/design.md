...
- [User scenarios](#user-scenarios)
  - [1. Solo developer — local-only, no Git](#1-solo-developer--local-only-no-git)
  - [2. Single team — one centralized catalog](#2-single-team--one-centralized-catalog)
  - [3. Multiple teams — per-team catalogs layered onto a global catalog](#3-multiple-teams--per-team-catalogs-layered-onto-a-global-catalog)
  - [4. External catalog composition (vendor / customer)](#4-external-catalog-composition-vendor--customer)
...
The smallest supported unit. One person, files on disk, no Git repository, no remote catalog, no providers. They can't share their config with anyone else because there's nothing to push to — but they're alone, so sharing isn't a requirement.

- **Collision risk:** zero — there's only one author and nothing to compose.
- **Value of scoping:** trivial — one local scope, never crossed.
- **Failure mode that matters most:** silently broken local config. A typo in `air.json`, a missing index file, a referenced skill that doesn't exist — there's no second pair of eyes and no Git history to bisect against.

```json
{
  "name": "solo-dev",
  "description": "One person, no Git, just files on disk",
...
What AIR's defaults need to do here: validate eagerly and fail loudly. With no peer review and no version control to roll back to, the tool has to be the safety net.

### 2. Single team — one centralized catalog

A team that maintains one shared catalog in a single Git repo. Everyone consumes the same catalog, everyone is at least aware of what's in it, and the team is small and coordinated enough that a single global namespace works — naming collisions get caught in PR review against the shared repo before they reach `main`.

- **Collision risk:** low — accidental collisions are caught by review; intentional same-name redefinitions are visible in the diff.
- **Value of scoping:** moderate — there's only one consumed scope (`@team/team-catalog`), so scope rarely surfaces day-to-day, but anything pulled in from elsewhere later would clearly be distinguishable.
- **Failure mode that matters most:** an artifact that quietly diverges from team consensus. Someone authors a new skill or replaces an existing one in a side branch and forgets it; if it merges, downstream behavior silently changes for everyone consuming the catalog.

```json
{
  "name": "single-team",
  "description": "Team consuming one shared catalog",
  "extensions": ["@pulsemcp/air-provider-github"],
  "catalogs": [
    "github://team/team-air-config"
  ]
}
```
...
A larger group where one shared catalog stops working. Different teams want their own catalogs (separate Git repos, or at least separate projects with non-overlapping scope), and each team layers their catalog on top of a single global catalog that holds the shared baseline. This is the most complex internal scenario — multiple per-team catalogs plus one org/global catalog.

- **Collision risk:** medium — most artifacts come cleanly from one team's catalog, but intentional collisions happen (a team writes a more specific variant of a global skill for their stack) and accidental cross-team shortname collisions are possible too.
- **Value of scoping:** high — `@team-a/team-a-catalog/...`, `@team-b/team-b-catalog/...`, and `@org/global/...` all need to coexist without conflating same-shortname artifacts across teams.
- **Failure mode that matters most:** silent merge in unexpected order. Two layers contribute the same shortname, the agent picks one based on resolution order, and there's no visible record of which won or why.

A team's `air.json` lists the global catalog plus their own:

```json
{
  "name": "team-a",
  "description": "Team A's catalog layered on the org global catalog",
  "extensions": ["@pulsemcp/air-provider-github"],
  "catalogs": [
    "github://acme/global-catalog",
    "github://acme/team-a-catalog"
  ]
}
```

What AIR's defaults need to do here: keep cross-team scopes visibly distinct, surface intentional-vs-accidental collisions distinctly, and make provenance obvious. Agents reading descriptions need enough context to pick the right artifact; humans reviewing config need to see who contributed what.

### 4. External catalog composition (vendor / customer)
...
**Decision.** Each AIR-managed target directory has a manifest stored at `<airHome>/manifests/<sha256(targetDir)>.json` (default `airHome` is `~/.air`, overridable via `AIR_HOME`). The manifest records which skills, hooks, and MCP servers AIR previously wrote into that directory. On every `prepareSession`, the adapter diffs the prior manifest against the new selection and removes artifacts that AIR previously wrote but that aren't selected anymore — `.claude/skills/<id>/`, `.claude/hooks/<id>/`, and the matching `.mcp.json` entries. User-authored entries (anything not in the manifest) are preserved. Because everything in the manifest gets deleted, an adapter records only what it created: a skill or hook directory, or an MCP server key, that already existed is left out (and left as it is), even when a catalog entry with the same ID is selected. Manifests written from `version: 2` on guarantee this for skills, and from `version: 3` on for MCP server keys; older manifests predate those guards ([#168](https://github.com/pulsemcp/air/issues/168), [#174](https://github.com/pulsemcp/air/issues/174)), so their skill and MCP server entries are re-checked against the catalog once before being trusted. Tracked in [#87](https://github.com/pulsemcp/air/issues/87).
