...
- [User scenarios](#user-scenarios)
  - [1. Solo developer or two-person team — single scope](#1-solo-developer-or-two-person-team--single-scope)
  - [2. Small team with shared scope plus a local in-flight layer](#2-small-team-with-shared-scope-plus-a-local-in-flight-layer)
  - [3. Mid-size org without a Developer Experience team](#3-mid-size-org-without-a-developer-experience-team)
  - [4. Mid-size org with a Developer Experience team](#4-mid-size-org-with-a-developer-experience-team)
  - [5. External catalog composition (vendor / customer)](#5-external-catalog-composition-vendor--customer)
  - [6. Open-source / community catalog](#6-open-source--community-catalog)
...
These are the team and org shapes AIR's defaults are designed for, ordered roughly from smallest unit to largest collision surface. They're a starting point, not the final list — candidates worth considering later include large enterprises with multiple orgs sharing a meta-catalog and agent-platform vendors shipping AIR-as-config.

### 1. Solo developer or two-person team — single scope

The smallest supported unit. One or two people working on one project, where everyone is effectively the same person and the entire catalog has uniform scope. No remote catalog, no providers, no cross-team layering — just a local `air.json` pointed at local index files.

- **Collision risk:** essentially none — the same human (or two humans who talk constantly) is authoring everything.
- **Value of scoping:** minimal in practice. Scope still exists conceptually (`local`), but the user never has to think about it.
- **Failure mode that matters most:** silently broken config. A typo in `air.json`, a missing index file, a referenced skill that doesn't exist — there's no second pair of eyes to catch it before it hits a session.

```json
{
  "name": "solo-dev",
  "description": "One person, one catalog",
...
What AIR's defaults need to do here: validate eagerly and fail loudly. A solo user has no peer review safety net, so the tool has to be the safety net.
...
- **Failure mode that matters most:** a local in-flight artifact silently overrides the team version for one engineer who forgot they had it. They debug a behavior they "see" but their teammate can't reproduce — because the local layer is shadowing the shared one with no visible signal.

```json
{
  "name": "team-dev",
  "description": "Shared team catalog plus a local workshop layer",
...
- **Failure mode that matters most:** flat-namespace collision. Two teams ship a `deploy-staging` skill with different semantics, an agent picks one based on resolution order, and the other team's expected behavior silently disappears.

```json
{
  "name": "team-a-config",
  "description": "Team A's catalog plus a borrowed catalog from Team B",
  "extensions": ["@pulsemcp/air-provider-github"],
  "catalogs": [
    "github://acme/team-b-catalog",
    "./team-a-catalog"
  ]
}
```

What AIR's defaults need to do here: keep cross-team scopes visibly distinct so two same-shortname artifacts don't get conflated. Without a central owner to mediate naming, the tool itself has to make scope obvious by default.

### 4. Mid-size org with a Developer Experience team
...
### 5. External catalog composition (vendor / customer)
...
### 6. Open-source / community catalog
...
> Same caveat as scenario 5: `exclude` and the `@scope/id` qualified-identifier syntax ship with the next major bump.
...
**Context.** Without scope, OSS composition ([scenario 6](#6-open-source--community-catalog)) is fundamentally fragile — two `code-review` skills from two authors are not the same artifact but get conflated by a flat ID space. Mid-size orgs without a central naming broker ([scenario 3](#3-mid-size-org-without-a-developer-experience-team)) hit the same flat-namespace collision when two teams ship a same-shortname skill in parallel. Mid-size orgs with a DevEx team ([scenario 4](#4-mid-size-org-with-a-developer-experience-team)) and vendor composition ([scenario 5](#5-external-catalog-composition-vendor--customer)) need provenance: when an agent reads a description, the qualified ID tells it (and the human reviewing config) where the entry came from. Scoping also turns previously-silent same-ID collisions into a hard-fail with a clear message.
